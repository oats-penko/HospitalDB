...
/*Q2: For a given division manager (say, ID = 200), report all regular employees 
...
/*for patients with 0 visits, reports count as 0 */ 
SELECT P.SSN AS SSN, P.firstName as firstName, P.lastName as lastName, q.NumVisits as NumVisits
FROM Patient P, 
...
UNION
SELECT p2.SSN AS SSN, p2.firstName as firstName, p2.lastName as lastName, 0 as NumVisits
	FROM Patient p2
	WHERE NOT EXISTS (SELECT Ap.patientSSN
	               FROM Appointment Ap
	                WHERE p2.SSN = Ap.patientSSN);
